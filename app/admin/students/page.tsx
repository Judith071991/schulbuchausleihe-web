...
export default function AdminStudentsPage() {
  ...
  const [religion, setReligion] = useState<StudentRow['religion']>('EV');
  const [course, setCourse] = useState<StudentRow['course']>(null);
  const [active, setActive] = useState(true);

  // Scan (mehrere Codes per Komma)
  ...
  async function loadClasses() {
    const { data, error } = await supabase.from('sb_classes').select('class_id').order('class_id', { ascending: true });
    if (!error && data) {
  ...
  async function assignBookToStudent() {
    ...
    try {
      ...
      for (const code of codes) {
        ...
        if (error) errList.push(`${code}: ${error.message}`);
        else okList.push(code);
      }
      ...
      if (errList.length && !okList.length) setMsg(parts.join('\n\n'));
      else setOk(parts.join('\n\n'));
  ...
  // NEU: Entsorgen / Code freigeben (disposed=true)
  async function disposeBookCodes() {
    setMsg(null);
    setOk(null);
    ...
    try {
      ...
      // optionaler Hinweis (einmal für alle)
      const note = prompt('Entsorgen: Notiz (optional):', 'kaputt/entsorgt') ?? '';
      ...
      for (const code of codes) {
        const { error } = await supabase.rpc('sb_admin_dispose_book', {
          p_book_code: code,
          p_note: note || null,
        });
      ...
      const parts: string[] = [];
      if (okList.length) parts.push(`🗑️ Entsorgt (Codes frei): ${okList.join(', ')}`);
      if (errList.length) parts.push(`❌ Fehler:\n- ${errList.join('\n- ')}`);
      ...
      setMsg(e?.message ?? 'Unbekannter Fehler beim Entsorgen.');
    } finally {
      setScanBusy(false);
    }
  }
      ...
      <div className="card">
        ...
        <div className="row">
          <label className="badge" style={{ cursor: 'pointer' }}>
            <input type="checkbox" checked={isGu} onChange={(e) => setIsGu(e.target.checked)} style={{ marginRight: 8 }} />
          ...
          <select className="select" value={course ?? ''} onChange={(e) => setCourse((e.target.value || null) as any)} style={{ maxWidth: 240 }}>
        ...
        <div className="small" style={{ marginTop: 6, opacity: 0.8 }}>
          Tipp: Du kannst mehrere Codes eingeben: <b>123,456,789</b>
        </div>
        <div style={{ height: 8 }} />
        <div className="row" style={{ flexWrap: 'wrap', gap: 10 }}>
          ...
          <button className="btn secondary" onClick={disposeBookCodes} disabled={scanBusy}>
            {scanBusy ? 'Arbeite…' : '🗑️ Entsorgen (Code frei)'}
          </button>
        </div>
        ...
        <div className="row">
          ...
          <div className="spacer" />
          <button className="btn secondary" onClick={loadClasses}>
        ...
        <div className="row" style={{ flexWrap: 'wrap', gap: 10 }}>
          ...
          <select className="select" value={moveToClassId} onChange={(e) => setMoveToClassId(e.target.value)} style={{ maxWidth: 200 }}>
            {classes.length === 0 ? <option value={moveToClassId || ''}>{moveToClassId || '(keine Klassen geladen)'}</option> : null}
            {classes.map((c) => (
